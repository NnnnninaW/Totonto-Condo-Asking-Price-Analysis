...
Then, we need to see whether there are problematic observations such as Leverage points, Outliers, and Influential points to fully understand why the estimated regression line is being placed where it is. Finally, we would go over the automated selection processes to learn about what are the suggested models. Combining contextual reasons, we would generate three preferred models for validation. We would fit them in our test dataset to compare their properties, understand potential limitations, and choose final model. At the end, instead of modifying our previous analysis, we would conclusively discuss our results with observed limitations.

```{r, include=FALSE}
#Create train and test data set 50/50, and prepare for validation at the end
set.seed(1)
...
#Check if the train dataset and test dataset are capturing similar observations
...
ggsave("condition2_rechecked.png", width = 15, height = 15, units = "in")
...
stepAIC(lm(asking_price ~ ., data=new), direction="both", k = 2)
stepAIC(lm(asking_price ~ ., data=new), direction="both", k = log(nrow(new)))
